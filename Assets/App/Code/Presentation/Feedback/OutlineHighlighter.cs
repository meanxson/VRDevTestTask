...
{
    /// <summary>
    /// Подсветка объёмного предмета контуром. Материал обводки добавляется
    /// к рендерерам объекта на время подсветки и убирается после — исходные
    /// материалы не трогаются.
    ///
    /// Для плоских напольных зон не годится: контур диска сверху вырождается
    /// в невидимую линию, там нужен ZoneMarkerHighlighter.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class OutlineHighlighter : TargetHighlighter
    {
        ...
        [Tooltip("Толщина контура в метрах — она одинакова при любом масштабе объекта.")]
        [Range(0.001f, 0.05f)]
        [SerializeField] private float _width = 0.012f;
